                ...
                <div key={course.id} className="bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 group">
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-gray-600 text-sm mb-4 flex items-center gap-2">
                      <Users className="w-4 h-4" /> {course.creatorName}
                    </p>
                    <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                      <div className="flex items-center gap-4 text-xs text-gray-500 font-medium">
                        <span className="flex items-center gap-1">
                          <BookOpen className="w-4 h-4 text-orange-400" /> {course.lessonCount} bài giảng
                        </span>
                        <span className="flex items-center gap-1">
                          <Users className="w-4 h-4 text-blue-400" /> {course.studentCount} học viên
                        </span>
                      </div>
                      <Link 
                        to={`/course/${course.id}`}
