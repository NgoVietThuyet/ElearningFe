...
export default function Home() {
  ...
  const benefitItems = [
    { title: "Học mọi lúc, mọi nơi", text: "Truy cập bài giảng trên mọi thiết bị, học tập linh hoạt 24/7.", icon: Clock3 },
    { title: "Lộ trình cá nhân hóa", text: "Xây dựng lộ trình học phù hợp với mục tiêu và trình độ của bạn.", icon: Target },
    { title: "Nội dung chuẩn học thuật", text: "Bài giảng được biên soạn bởi giảng viên giàu kinh nghiệm.", icon: Award },
    { title: "Hỗ trợ tận tâm", text: "Đội ngũ hỗ trợ luôn sẵn sàng giải đáp mọi thắc mắc của bạn.", icon: MessageSquare },
  ];
    ...
    <main className="min-h-screen bg-[#f8fafc] text-[#0f172a]">
      <div className="mx-auto w-full max-w-[1280px] px-4 py-6 lg:px-8">
          ...
          <div className="relative grid items-center gap-8 xl:grid-cols-[1.02fr_0.98fr]">
            <div className="max-w-[620px]">
              ...
              <h1 className="max-w-[620px] text-[38px] font-black leading-[1.08] tracking-tight text-[#0f172a] md:text-[54px]">
                Khám phá thế giới
                <span className="block text-[#ff4f12]">Sinh học đầy kỳ diệu</span>
              </h1>

              <p className="mt-5 max-w-[560px] text-[15px] font-semibold leading-7 text-slate-600">
                Học tập dễ hiểu, trực quan và hiệu quả với đội ngũ giảng viên chuyên môn cao và nội dung chuẩn đại học.
              </p>

              <div className="mt-7 flex flex-wrap items-center gap-3">
                <Link to="/courses" className="inline-flex h-12 items-center gap-3 rounded-[8px] bg-[#ff4f12] px-6 text-sm font-black text-white shadow-lg shadow-orange-500/20 transition hover:bg-[#e9460e]">
                  Khám phá khóa học <ArrowRight className="h-4 w-4" />
                </Link>
                <Link to="/teachers" className="inline-flex h-12 items-center rounded-[8px] border border-slate-200 bg-white px-6 text-sm font-black text-[#0f172a] shadow-sm transition hover:bg-slate-50">
                  Tìm giảng viên
                </Link>
              </div>
              ...
              <div className="mt-7 flex items-center gap-4">
                <div className="flex -space-x-3">
                  {featuredTeachers.slice(0, 4).map((teacher) => (
                    <TeacherAvatar key={teacher.id} teacher={teacher} className="h-10 w-10 rounded-full border-2 border-white shadow-sm" />
                  ))}
                  {featuredTeachers.length === 0 && (
                    <div className="flex h-10 w-10 items-center justify-center rounded-full border-2 border-white bg-orange-50 text-xs font-black text-orange-600">ES</div>
                  )}
                </div>
              ...
              <div className="absolute -left-3 bottom-8 hidden rounded-[8px] bg-white/95 p-4 shadow-xl ring-1 ring-slate-100 backdrop-blur md:block">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-500">{formatNumber(stats?.totalLessons)}+ bài giảng</p>
                  </div>
                </div>
              </div>
              ...
              <div className="absolute -right-4 top-8 hidden space-y-4 xl:block">
                <div className="flex w-56 items-center gap-3 rounded-[8px] bg-white/95 p-4 shadow-xl ring-1 ring-slate-100 backdrop-blur">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-500">Mọi lúc, mọi nơi</p>
                  </div>
                </div>
                <div className="flex w-56 items-center gap-3 rounded-[8px] bg-white/95 p-4 shadow-xl ring-1 ring-slate-100 backdrop-blur">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-500">Học đúng, tiến bộ nhanh</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <section className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {statCards.map((item) => (
            <div key={item.label} className="rounded-[8px] border border-slate-100 bg-white p-6 shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="mt-2 inline-flex rounded-full bg-emerald-50 px-2 py-1 text-[11px] font-black text-emerald-600">↑ {item.delta}</p>
                  <p className="mt-1 text-xs font-semibold text-slate-400">So với tháng trước</p>
                </div>
              </div>
            </div>
          ))}
              ...
              {learningPath.map((item, index) => {
                const content = (
                  ...
                  <article className="rounded-[8px] border border-slate-100 bg-white p-5 text-center shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                    ...
                    <h3 className="mt-5 text-lg font-black text-[#0f172a]">{item.title}</h3>
                    <p className="mx-auto mt-3 min-h-[44px] max-w-[210px] text-sm font-semibold leading-6 text-slate-500">{item.text}</p>
                    <div className={`mx-auto mt-5 inline-flex items-center gap-2 rounded-full px-3 py-2 text-xs font-black ${item.tone}`}>
            ...
            <section className="mt-5 rounded-[8px] border border-slate-100 bg-white p-5 shadow-sm lg:p-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                {featuredCourses.map((course, index) => (
                  <Link key={course.id} to={`/course/${course.id}`} className="group overflow-hidden rounded-[8px] border border-slate-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
                    ...
                    <div className="p-4">
                      <h3 className="line-clamp-1 text-base font-black text-[#0f172a] group-hover:text-[#ff4f12]">{course.title}</h3>
                ...
                {featuredCourses.length === 0 && <EmptyState text="Chưa có khóa học để hiển thị." />}
              </div>
            </section>
            ...
            <section className="mt-5 rounded-[8px] border border-slate-100 bg-white p-5 shadow-sm lg:p-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                {featuredTeachers.map((teacher) => (
                  <div key={teacher.id} className="overflow-hidden rounded-[8px] border border-slate-100 bg-white text-center shadow-sm">
                    <TeacherAvatar teacher={teacher} className="h-44 w-full bg-slate-50" />
                    <div className="p-4">
                      <h3 className="text-base font-black">{teacher.fullName}</h3>
                      <p className="mt-1 line-clamp-1 text-sm font-semibold text-slate-500">{teacher.email}</p>
                      <div className="mt-3 flex items-center justify-center gap-2 text-xs font-bold text-amber-500">
                        <Star className="h-3.5 w-3.5 fill-current" />
                        4.9
                        <span className="text-slate-400">({formatNumber(teacher.lessonCount)} bài giảng)</span>
                      </div>
                    </div>
                  </div>
                ))}

                {featuredTeachers.length === 0 && <EmptyState text="Chưa có giảng viên để hiển thị." />}
              </div>
            </section>
                ...
                {featuredNews ? (
                  <div className="grid gap-4">
                    {[featuredNews, ...sideNews].map((item, index) => (
                      <Link key={item.id} to={`/news/${item.id}`} className="grid gap-4 rounded-[8px] border border-slate-100 p-3 transition hover:bg-slate-50 md:grid-cols-[130px_1fr]">
                        <div className={index === 0 ? "h-24 overflow-hidden rounded-[8px]" : "h-20 overflow-hidden rounded-[8px]"}>
                          <NewsImage item={item} />
                        </div>
                        <div>
                          <h3 className="line-clamp-2 text-sm font-black md:text-base">{item.title}</h3>
                          <p className="mt-2 line-clamp-2 text-sm font-semibold leading-6 text-slate-500">{stripHtml(item.content)}</p>
        ...
        <footer className="mt-5 rounded-[8px] border border-slate-100 bg-white p-6 shadow-sm">
          <div className="grid gap-8 md:grid-cols-[1.2fr_0.8fr_0.8fr_1fr]">
            ...
            <FooterLinks title="Khám phá" items={[["Khóa học", "/courses"], ["Giảng viên", "/teachers"], ["Tin tức", "/news"], ["Về chúng tôi", "/about"]]} />
            <FooterLinks title="Hỗ trợ" items={[["Trung tâm trợ giúp", "/feedback"], ["Hướng dẫn sử dụng", "/feedback"], ["Chính sách bảo mật", "/feedback"], ["Điều khoản sử dụng", "/feedback"]]} />
            <div>
...
function SectionHeader({ title, to, compact = false }: { title: string; to: string; compact?: boolean }) {
  return (
    <div className={compact ? "mb-5 flex items-center justify-between gap-4" : "mb-5 flex items-center justify-between gap-4"}>
      <h2 className="text-xl font-black tracking-tight">{title}</h2>
      <Link to={to} className="inline-flex items-center gap-2 text-sm font-black text-slate-600 hover:text-[#ff4f12]">
